...
class MoleculeDefWindow(wx.Frame):
    ...
    def __init__(self):
        ...
        self.pot_atomtypes_list = []
        for k, v in self.pot_atomtypes.iteritems():
            self.pot_atomtypes_list.append(v[0] + "[" + v[1] + "]")
        # They are initially empty until a molecule is selected
        self.mol_atomtypes = None
        self.map_pot_id2index = None
        self.mol_tags = []
        ...
        self.toggle_checkbox = wx.CheckBox(panel, id=wx.ID_ANY, label="Toggle names")
        ...
        topRightHalfSizer.Add(self.toggle_checkbox, 0, wx.ALL, 5)
    ...
    def _on_apply_button(self, event):
        ...
        if self.radiobox_sel_mouse.GetValue():
            ...
            atoms_changed = self.pymol_cmd.change_atom_name_cond(atom_name_in, atom_name_out, expression, pot_atomtype_id=atomtype_id, color=self.pymol_cmd.get_random_color())
...
class AtomListCtrl(wx.ListCtrl):
    ...
    def fill_from_mol_model(self, mol_model, pot_atomtypes):
        ...
        for atom in mol_model.atom:
            ...
            if (atom.resn in pot_atomtypes.keys()):
                self.SetStringItem(index, 1, atom.resn)
                self.SetStringItem(index, 2, pot_atomtypes[atom.resn][1])
            else:
